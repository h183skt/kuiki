/**
 * 区域マンション一覧 ウェブアプリ（webapp.gs v4.2.1 訪問記録 入力対応・安全チェック追加）
...
function buildHtml_(dataJson, colorsJson, resultsJson) {
  return '<!DOCTYPE html><html lang="ja"><head><meta charset="UTF-8">' +
    ...
    '<div class="toggle"><button id="bList" class="on">一覧</button><button id="bMap">地図</button></div></div>' +
    ...
    'let curArea="";let curQ="";let mode="list";let map=null;let layer=null;' +
    ...
    'let initMode="list";' +
    'try{const st=JSON.parse(sessionStorage.getItem("st")||"{}");curArea=st.a||"";curQ=st.q||"";if(st.v)savedView=st.v;if(st.m==="map")initMode="map";}catch(e){}' +
    ...
    'if(initMode==="map"){setMode("map");}else{render();}' +
    '</script></body></html>';
}
